...
export type FaucetReq = { to: string; amount: number }
...
export const postFaucet = (body: FaucetReq) => api.post<{ status: string; user: string; balance: number }>(`/faucet`, body).then(r => r.data)
